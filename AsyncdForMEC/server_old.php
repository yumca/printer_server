...
class serv_fuc extends Base
{
    ...
    public function servreceive($serv, $fd, $reactor_id, $data){
        ...
        if(!empty($data)){
            ...
            $res_package = null;
            //判断pmm发送包信息
            if($head['HeadFlags'][1] == '66ee66ee'){
                switch($head['CmdCode'][1]){
                    case '01': //pmm请求登录
                        $type = 'login';
                        $package_data = PrintCtr::parse_login($data);
                        //todo
                        //存储登录信息
                        $package_data['ctime'] = microtime();
                        $reserv->addHashval($this->keys['link_key'], $fd, $package_data);
                        $reserv->addHashval($this->keys['link_uuidkey'], $package_data['UUID'], $fd);
                        if(is_numeric($package_data['HalErrCode'][1]) && intval($package_data['HalErrCode'][1]) === 0){
                            //硬件无错打包回应数据
                            $res_package = PrintCtr::put_pack($head, $package_data);
                            var_dump(date('Y-m-d H:i:s').',serverGET:'.$fd.',login:'.$package_data['UUID'][1]);
                        }
                        break;
                    case '84': //AT应答指令
                        $type = 'AT';
                        $package_data = PrintCtr::parse_at($data);
                        //todo  根据应答做出相关动作
                        $package_data['atres'] = trim($package_data['atres']);
                        if($package_data['atres'] == 'AT Comand Error'){
                            //at指令错误
                            var_dump('AT:' . $package_data['atres']);
                        }else if(strstr($package_data['atres'], 'OK:') !== false){
                            //根据链接标识寻找队列
                            $taskdata = $reserv->pop($this->keys['task_list0'].$fd);
                            if(!$taskdata){
                                var_dump('未找到AT队列:' . $package_data['atres'].';fd:'.$fd);
                                return;
                            }
                            switch ($taskdata[0]){
                                case 'check_online':
                                    $uuid = trim(str_replace('OK:','',$package_data['atres']));
                                    $linkdata = json_decode($reserv->getoneHashval($this->keys['link_key'], $fd), true);
                                    if($linkdata['UUID'][1] != $uuid){
                                        var_dump('redis-uuid:' . $linkdata['UUID'][1] . ';send-uuid:'.$uuid);
                                    }else{
                                        //更新数据库
                                        var_dump('更新数据库');
                                    }
                            ...
                            var_dump('AT:' . $package_data['atres'].';fd:'.$fd);
                        }
                        break;
                    case '07': //打印数据传输指令
                        $type = 'print';
                    ...
                    case '09':  //报警指令  无需应答
                        $type = 'warning';
                        $package_data = PrintCtr::parse_warning($data);
                        //获取链接数据
                        $linkdata = json_decode($reserv->getoneHashval($this->keys['link_key'],$fd), true);
                        $linkdata['PrinterStatus'][1] = $package_data['PrinterStatus'][1];
                        //更新redis数据
                        $reserv->addHashval($this->keys['link_key'], $fd, $linkdata);
                ...
                if(empty($tmpprintdata)){
                    ...
                    $datalen = strlen($tmpprintdata['DataPackBuf'][1]);
                    ...
                    if($tmpprintdata['DataPackeLen_oct'] <= $datalen){
                        ...
                        $res_package = PrintCtr::put_pack($tmpprintdata, $tmpprintdata);
                    }
                }
            }
...
//        $reserv->redis()->lTrim($this->keys['task_list0'].$fd,0,$llen);
        ...
        $reserv->delHashval($this->keys['link_uuidkey'], $pack['UUID']);
        unset($reserv);
